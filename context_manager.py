...
class ChatContextManager:
    ...
    def get_recent_assistant_messages(self, chat_id, limit=5):
        """
        获取最近的assistant消息（机器人发送的消息）
        ...
        Returns:
            list: 包含消息字典的列表，每个字典包含 'content' 字段
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        try:
            ...
            messages = [{"content": content[0]} for content in cursor.fetchall()]
            return messages
        except Exception as e:
            logger.error(f"获取最近assistant消息时出错: {e}")
            return []
        finally:
            conn.close() 